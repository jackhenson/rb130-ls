...
# called any? and all? that are similar to the standard
# Enumerable methods with the same names. In this exercise,
# you will develop another of the methods in this family,
# none?.

# Enumerable#none? processes elements in a collection
# by passing each element value to a block that is
# provided in the method call. If the block returns
# true for any element, then #none? returns false.
# Otherwise, #none? returns true. Note in particular
# that #none? will stop searching the collection the
# first time the block returns true.

# Write a method called none? that behaves similarly
# for Arrays. It should take an Array as an argument,
# and a block. It should return true if the block
# returns false for all of the element values. Otherwise,
...
# from the Array and Enumerable classes:
# all?, any?, none?, one?.
